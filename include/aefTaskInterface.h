 ...
 *   (and willing).
 */
class aefTaskInterface
{
    // Class Methods
public:

    virtual AEF_STATUS Start() = 0;
    virtual AEF_STATUS Stop(uint32_t msWait = AEF_WAIT_FOREVER) = 0;
    virtual AEF_STATUS Stop(CAefQueueTask* pTask, uint32_t msgID) = 0;
    ...
    virtual void TaskDelay(uint16_t ms) = 0;
...
protected:
    ...
    virtual bool WatchdogCheck(uint32_t *task_id = NULL) = 0;
