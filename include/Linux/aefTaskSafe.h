...
#include "aefTaskSafeInterface.h"
#include "aefMutex.h"

#define AEF_LOCK_TIMEOUT	100		// a "useful" timeout value? (in ms)

class CAefTaskSafe : public CAefMutex, public aefTaskSafeInterface
{
	// Class Methods
public:
	CAefTaskSafe();  
	virtual ~CAefTaskSafe();
	virtual bool TakeOwnership(uint32_t msWait = AEF_WAIT_FOREVER);
