 ...
 * \param  bufSize      [uint32_t] Maximum number of bytes that can be sent to pClientObj at once
...
AEF_STATUS CAefSerial::Configure(const char* pDevName,
                                 CAefQueueTask* pTask, uint32_t dataRcvdId, uint32_t shutdownId,
                                 aefPacketInterface* pPktObj, uint32_t bufSize, int timeout,
                                 int keepalive)
{
 ...
 *
 ***************************************************************************/
AEF_STATUS CAefSerial::ConfigurePort(uint32_t baudRate, uint8_t dataBits, uint8_t parity, uint8_t stopBits)
{
    bool bUpdate = false;
 ...
 *
 ***************************************************************************/
AEF_STATUS CAefSerial::Send(uint8_t *pData, uint32_t dataLen)
{
    AEF_STATUS result = AEF_ERROR;
    uint8_t* pPkt;
    uint32_t pktLen;
...
*
***************************************************************************/
uint32_t CAefSerial::TicksFromMs(uint32_t ms)
{
...
*
***************************************************************************/
uint32_t CAefSerial::TicksFromTOD()
{
    struct timeval tv;
    gettimeofday(&tv, 0);
